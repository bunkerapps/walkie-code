...
export const VOICE_STYLE =
  ...
  'Es una transcripción automática: si una parte no tiene sentido o parece estar en otro idioma, ' +
  'no actúes sobre esa parte; si puede cambiar lo que hay que hacer, preguntá primero qué quiso decir, citándola.';

// Letras que no existen en el idioma del dictado: si aparecen, Whisper seguramente alucinó
// (pasa sobre todo con el silencio o el ruido del final). Por ahora, solo para español.
const FOREIGN = { es: /[^\s\p{P}\p{N}a-záéíóúüñ]/iu };

// Fragmento sospechoso del dictado (desde la primera palabra rara hasta el final), o null.
export function garbledTail(prompt, language = 'es') {
  const foreign = FOREIGN[language];
  if (!foreign) return null;
  const words = String(prompt ?? '').trim().split(/\s+/);
  const first = words.findIndex((w) => foreign.test(w));
  return first === -1 ? null : words.slice(first).join(' ');
}

// Instrucción completa para Claude: el estilo de voz y, si hace falta, el aviso de la parte dudosa.
export function voiceContext(prompt, language) {
  const tail = garbledTail(prompt, language);
  if (!tail) return VOICE_STYLE;
  return `${VOICE_STYLE} Ojo: esta parte del dictado parece mal transcripta: «${tail}». ` +
    'Antes de trabajar, preguntale al usuario qué quiso decir ahí (salvo que el resto del mensaje ya sea claro y esa parte no cambie nada).';
}
