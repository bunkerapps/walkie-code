...
export async function saveImage(dir, buffer) {
  ...
  const file = path.join(dir, `walkie-code-${id}.${ext}`);
...
export async function cleanupImages(dir, maxAgeMs = MAX_IMAGE_AGE_MS, now = Date.now()) {
  ...
  for (const name of names) {
    if (!/^walkie-code-[a-f0-9]{16}\.\w+$/.test(name)) continue;
...
// Lo dictado y, al final, la ruta de cada foto (con los espacios escapados, como los espera la terminal).
export function promptWithImages(text, files) {
  const clean = String(text || '').replace(/\s+/g, ' ').trim() || (files.length > 1 ? 'Mirá estas imágenes' : DEFAULT_IMAGE_TEXT);
  return [clean, ...files.map((f) => f.replace(/ /g, '\\ '))].join(' ');
}

export const promptWithImage = (text, file) => promptWithImages(text, [file]);
