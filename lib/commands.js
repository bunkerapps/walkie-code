...
// Muletillas al arrancar a hablar: "bien, cambiame al canal 4", "dale, canal superprecio".
const FILLER = /^(?:(?:bien|bueno|buenas|dale|ok|okey|okay|listo|perfecto|genial|joya|che|a ver|ahora|entonces|y)\s+)+/;
...
export function parseChannelCommand(text) {
  const t = normalize(text).replace(FILLER, '');
