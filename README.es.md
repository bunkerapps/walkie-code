...
- **＋** abre un explorador de carpetas dentro de `projectsRoot`. Con **PROYECTO NUEVO** se crea una carpeta con una página inicial y se abre su propio canal de Claude Code, listo para mostrarlo en la tele. Los proyectos nacen dentro de `laboratorio` (configurable en `labFolder`): ahí todo es descartable y aparece marcado como PRUEBA. Dentro de un proyecto del laboratorio hay dos botones más: **ASCENDER**, que lo saca del laboratorio y lo convierte en un proyecto de verdad, y **BORRAR**, que pide confirmación y lo manda a la Papelera (nunca borra de verdad). Fuera del laboratorio, el walkie no borra ni mueve nada, y tampoco borra un proyecto con un canal abierto.
